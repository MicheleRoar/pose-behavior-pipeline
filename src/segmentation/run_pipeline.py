...
def _sanitize_timestamp(ts: str) -> str:
    """Digits only, e.g. '00:22:34' -> '002234'. Collision-free: two
    different --ss/--to strings only collide if they differ by
    punctuation only, which never happens for ffmpeg's timestamp syntax."""
...
def _transcode(video_path: str, ss: str | None, to: str | None, out_path: Path, overwrite: bool) -> str:
    ...
    `[ss, to]` if given, full video otherwise): normalizes source `.mkv`
    codecs cv2/psifx don't decode cleanly on their own. Returns the path
    used as input to the rest of the pipeline."""
...
def run_pipeline(
    *,
    ...
    use_osnet: bool = True,
    # overlap-heuristic pass-through (defaults match merge_fragments.py)
    overlap_pixel_gap_threshold: float | None = None,
    ...
    if _has_mask_files(paths["masks_dir"]) and not overwrite:
        ...
        from segmentation.run_sam3_baseline import run_sam3_baseline
    ...
    if paths["merge_report"].exists() and not overwrite:
        ...
        from segmentation.merging.merge_fragments import (
            DEFAULT_MERGE_THRESHOLD, DEFAULT_MIN_FRAGMENT_FRAMES,
            DEFAULT_POOLED_SAMPLES_PER_MEMBER, DEFAULT_SIGNATURE_SAMPLES,
            DEFAULT_OVERLAP_PIXEL_GAP_THRESHOLD, DEFAULT_OVERLAP_CENTROID_THRESHOLD,
            DEFAULT_MIN_OVERLAP_FRAMES, merge_fragments,
        )
        import json
        ...
        report = merge_fragments(
            ...
            overlap_pixel_gap_threshold=overlap_pixel_gap_threshold if overlap_pixel_gap_threshold is not None else DEFAULT_OVERLAP_PIXEL_GAP_THRESHOLD,
            overlap_centroid_threshold=overlap_centroid_threshold if overlap_centroid_threshold is not None else DEFAULT_OVERLAP_CENTROID_THRESHOLD,
            min_overlap_frames=min_overlap_frames if min_overlap_frames is not None else DEFAULT_MIN_OVERLAP_FRAMES,
...
def main() -> None:
    ...
    merge.add_argument("--overlap-pixel-gap-threshold", type=float, default=None)
    merge.add_argument("--overlap-centroid-threshold", type=float, default=None)
    merge.add_argument("--min-overlap-frames", type=int, default=None)
    merge.add_argument("--overlap-classifier", dest="overlap_classifier_path", default=None,
